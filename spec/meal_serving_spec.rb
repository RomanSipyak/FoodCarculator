...
describe MealServing do
  ...
  let(:recipe) { Recipe.new({ name: 'borsch', serving_count: 5 }, arr) }
  let(:meal_ser) { described_class.new(5, recipe) }

  describe 'constructor and setter' do
    context 'enter negative digit' do
      it 'negative in constructor' do
        expect{ described_class.new(-1, recipe) }.to raise_error(ArgumentError, 'you can`t enter on negative digit in constructor')
      end
